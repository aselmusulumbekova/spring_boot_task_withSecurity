package com.example.spring_boot_task_withsecurity.controllers;
...
public class VideoController {
    ...
    private String saveVideo(@PathVariable("lessonId")int id, @ModelAttribute("newVideo")Video video) {
        videoService.saveVideo(id,video);
        return "redirect:/videos/allVideos/ "+id;
    }
    @GetMapping("/getVideo/{videoId}")
    private String getVideoById(@PathVariable("videoId")int id,Model model) {
    ...
    private String deleteVideo(@PathVariable("lessonId")int id,@PathVariable("videoId")int videoId) {
